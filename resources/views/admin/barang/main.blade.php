  		...
  		<div class='col-lg-12 col-md-12 col-sm-12 col-xs-12' style='padding:0px'>
            ...
            <a href="{{ route($routePage.'Create') }}">
              <button type="button" class="btn btn-sm btn-primary" id="btn-add">
                <span class="fa fa-plus"></span> &nbsp Tambah Barang
              </button>
            </a>
  				</div>
  				<!-- Search -->
...
@section('js')
  <script type="text/javascript">
    var datagrid = $("#datagrid").datagrid({
      url                     : "{!! route('barangDatagrid') !!}",
      primaryField            : 'id_barang',
      rowNumber               : true,
      rowCheck                : false,
      ...
      {field: 'kode_barang', title: 'Kode', editable: false, sortable: true, width: 400, align: 'left', search: true},
      {field: 'nama_barang', title: 'Nama', editable: false, sortable: true, width: 400, align: 'left', search: true},
      {field: 'harga', title: 'Harga', editable: false, sortable: true, width: 400, align: 'left', search: true},
  ...
  tag += '<li><a href="{{ route($routePage) }}/edit/'+rowData.id_barang+'" class=" waves-effect waves-block m-l-5"><i class="fa fa-edit"></i> Edit</a></li>';
...
function detail(rowIndex) {
    $('.main-layer').hide();
    var id = datagrid.getRowData(rowIndex).id_barang;
  ...
  function deleted(rowIndex) {
    ...
    swal({
      ...
      if (willDelete) {
        $.post('{{ route($routePage.'Destroy') }}',{id:rowData.id_barang}).done(function(data) {
          if (data=='true') {
