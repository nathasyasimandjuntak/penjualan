  		...
  		<div class='col-lg-12 col-md-12 col-sm-12 col-xs-12' style='padding:0px'>
            ...
            <a href="{{ route($routePage.'Create') }}">
              <button type="button" class="btn btn-sm btn-primary" id="btn-add">
                <span class="fa fa-plus"></span> &nbsp Tambah Pelanggan
              </button>
            </a>
  				</div>
  				<!-- Search -->
...
@section('js')
  <script type="text/javascript">
    var datagrid = $("#datagrid").datagrid({
      ...
      {field: 'kode_pelanggan', title: 'Kode', editable: false, sortable: true, width: 400, align: 'left', search: true},
  ...
  tag += '<li><a href="{{ route($routePage) }}/edit/'+rowData.id_pelanggan+'" class=" waves-effect waves-block m-l-5"><i class="fa fa-edit"></i> Edit</a></li>';
...
function detail(rowIndex) {
    $('.main-layer').hide();
    var id = datagrid.getRowData(rowIndex).id_pelanggan;
  ...
  function deleted(rowIndex) {
    ...
    swal({
      ...
      if (willDelete) {
        $.post('{{ route($routePage.'Destroy') }}',{id:rowData.id_pelanggan}).done(function(data) {
          if (data=='true') {
